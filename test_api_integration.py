    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, result in enumerate(batch_result.get('results', [])):
                print(f"   • Image {i+1}: Drowsy={result.get('isDrowsy', 'N/A')}, "
...
def test_camera_service_simulation():
    ...
    for i in range(3):  # Simulate 3 captures
        ...
        try:
            start_time = time.time()
            response = requests.post("http://127.0.0.1:8000/api/detect", 
            ...
            if response.status_code == 200:
                ...
                print(f"   {status} | Confidence: {confidence:.1%} | Alert Level: {alert_level} | Time: {(end_time-start_time)*1000:.0f}ms")
                
                if alert_level >= 2:
                    print(f"   🚨 ALERT TRIGGERED! Driver appears drowsy (confidence: {confidence:.1%})")
            else:
